...
public class NoteSpawner : MonoBehaviour {
  ...
  void Start() {
    ...
    foreach (Tap tap in song.taps) {
      Instantiate(tapPrefab, new Vector3(tap.channel, 0, tap.start), Quaternion.identity);
    }
    foreach (Hold hold in song.holds) {
      GameObject obj = Instantiate(holdPrefab, new Vector3(hold.channel, 0, hold.start), Quaternion.identity);
