...
namespace Reilas
{
    public sealed class HoldNote : MonoBehaviour
    {
        ...
        public void Render(float currentTime, int noteNum, List<ReilasNoteLineEntity> noteList, List<SpeedChangeEntity> speedChangeEntities)
        {
            ...
            float headPos = NotePositionCalculatorService.GetPosition(_entity.Head.JudgeTime, currentTime, _noteSpeed, speedChangeEntities);
            float tailPos = NotePositionCalculatorService.GetPosition(_entity.Tail.JudgeTime, currentTime, _noteSpeed, speedChangeEntities);
